...
// Catch-all proxy de /api/research-bcra/* → backend (tab BCRA de la vista
// Research, módulo `research`). Mismo patrón que /api/research1816/[...path]:
// propaga la identidad para que el RBAC del backend decida por USER.
...
export async function GET(req: Request, { params }: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    const target = `${API_URL}/api/research-bcra/${suffix}${url.search}`;
