...
// NEGOCIO → CARTERAS · tab EVOLUCIÓN — cómo viene rindiendo la cuenta.
//
// Es la mitad de abajo de la vieja tab PORTAFOLIO: el gráfico del valor (o del
// rendimiento acumulado) y la tabla MENSUAL con el flujo neto, el PnL y la TEM /
// TEA de cada mes. La mitad de arriba (posiciones y auditoría por título) se
// fue a la tab ACTIVOS del informe, así que acá quedó SOLO la historia.
//
// Tiene tab propia y no un panel más adentro de RESUMEN por dos motivos: es la
// única pantalla que cuenta el RENDIMIENTO (todo lo demás del informe es una
// foto del día) y va a ser una hoja del PDF.
//
// Tres cosas que NO son obvias y ya costaron caro:
//   · El XIRR en USD usa un cashflow con el MEP de CADA fecha, no el ARS
//     dividido por un solo tipo de cambio. Por eso el backend manda columnas
//     `*_usd` paralelas y el toggle solo elige cuál mostrar.
//   · La fila del mes EN CURSO cierra al día de HOY (`live`) y se recalcula al
//     cerrar el mes: por eso va etiquetada.
//   · Clickear un mes despliega sus flujos INLINE y trae la atribución de la
//     variación (efecto mercado vs efecto operado) contra el mes anterior.
...
export function CarterasEvolucionView({ idCuenta, nombreCuenta }: Props) {
  const [mensualResp, setMensualResp] = useState<MensualResp | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  // Mes elegido en la tabla. null = ninguno. Con un mes elegido se despliegan
  // sus flujos inline y se pide la atribución de la variación contra el anterior.
  const [selectedFecha, setSelectedFecha] = useState<string | null>(null);
  const [movResp, setMovResp] = useState<MovimientosResp | null>(null);
  const [movLoading, setMovLoading] = useState(false);
  // Variación vs el cierre anterior — solo con un mes elegido.
  const [varResp, setVarResp] = useState<VariacionResp | null>(null);
  const [varLoading, setVarLoading] = useState(false);
  // Ventana del chart: "6M" default — 6 meses para que un outlier viejo no
  // apague el rango Y. "3M" / "1A" / "ALL" presets, ◀ ▶ para mover el offset.
  const [chartRango, setChartRango] = useState<"3M" | "6M" | "1A" | "ALL">("6M");
  const [chartOffset, setChartOffset] = useState<number>(0);
  // Un solo eje Y por vez (Valor $ o Rendimiento %): las dos juntas con doble
  // eje hacían la escala ilegible.
  const [chartMetric, setChartMetric] = useState<"valor" | "rendimiento">("valor");
  // ARS (default) o USD. El backend manda columnas `*_usd` paralelas con el MEP
  // de CADA fecha aplicado al cashflow del XIRR — el toggle solo elige cuál
  // mostrar, nunca divide por un tipo de cambio.
  const [moneda, setMoneda] = useState<"ARS" | "USD">("ARS");
  const esUSD = moneda === "USD";

  // La tabla mensual es la ÚNICA fuente del chart y de la grilla. La serie
  // DIARIA (`/serie`) ya no se pide: alimentaba el panel de posiciones, que se
  // fue a la tab ACTIVOS, y bajarla igual era una query por nada.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setLoading(true);
      ...
      try {
        const r = await fetch(
          `/api/valuaciones/${encodeURIComponent(idCuenta)}/mensual`, { cache: "no-store" });
        if (!r.ok) throw new Error(`mensual HTTP ${r.status}`);
        const m: MensualResp = await r.json();
        if (!cancelled) setMensualResp(m);
      } catch (e) {
  ...
  // Movimientos — solo cuando hay un mes elegido (sin mes el panel no se dibuja).
  //
  // NO se resetea el estado al deseleccionar: lo que decide qué se muestra es
  // `movDelMes`, que compara la respuesta con el mes elegido. Así no hay un
  // render de más por cada cambio de selección y, sobre todo, al saltar del mes
  // A al B no se ven un instante los flujos de A rotulados como B.
  useEffect(() => {
  ...
  // Variación vs el cierre anterior — mismo criterio que los movimientos.
  useEffect(() => {
    if (!selectedFecha) return;
    let cancelled = false;
    (async () => {
  ...
  // Una respuesta sirve solo si es del mes que está elegido AHORA. Comparar
  // contra el propio payload (y no contra un flag de loading) es lo que evita
  // que los flujos del mes anterior se muestren un instante bajo el título del
  // mes nuevo.
  const movDelMes = selectedFecha && movResp?.mes === selectedFecha.slice(0, 7) ? movResp : null;
  const varDelMes = selectedFecha && varResp?.fecha === selectedFecha ? varResp : null;
    ...
    <div className="h-full flex flex-col gap-2 p-2 overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex gap-2 overflow-hidden">
        ...
        <div className="w-1/2 border border-[var(--t-border)] bg-[var(--t-panel)] flex flex-col overflow-hidden min-w-0 min-h-0">
          <div className="flex items-center px-3 py-1.5 border-b border-[var(--t-border)] bg-[var(--t-brand)] shrink-0 gap-2">
            ...
            <div className="ml-auto inline-flex items-center gap-1">
              <button
                ...
                className="px-1 py-0 text-[10px] text-white/70 border border-white/40 hover:text-white hover:border-white disabled:text-white/25 disabled:border-white/15 disabled:cursor-not-allowed"
              >◀</button>
              <div className="inline-flex items-stretch border border-white/40 divide-x divide-white/40">
              ...
              <span className="text-[10px] text-white/70 font-mono">
                ...
                {chartMetric === "valor" ? (
                  <>
                    ...
                    <span className="text-white/60 ml-1">{esUSD ? "USD" : "ARS"}</span>
                  </>
                ) : (
        ...
        <div className="w-1/2 border border-[var(--t-border)] bg-[var(--t-panel)] flex flex-col overflow-hidden min-w-0 min-h-0">
          <div className="flex items-center px-3 py-1.5 border-b border-[var(--t-border)] bg-[var(--t-brand)] shrink-0 gap-2">
            ...
            <DownloadButton
              // Sobre el azul de la cabecera el gris del tema no se ve.
      ...
      {/* ATRIBUCIÓN — qué parte de la variación del mes fue PRECIO y qué parte
          fue haber comprado o vendido. Vivía adentro del panel PORTFOLIO como
          una sub-tab escondida; acá está donde se hace la pregunta: mirando el
          mes que se acaba de clickear. Sin mes elegido no se dibuja (necesita un
          cierre contra el cual comparar). */}
      {selectedFecha && (
        <div className="shrink-0 max-h-[38%] min-h-0">
          <Panel titulo={`Variación de ${fmtMesAnio(selectedFecha.slice(0, 7))} vs el cierre anterior`}
                 extra={varLoading
                   ? <span className="text-[10px] text-white/70">calculando…</span>
                   : <span className="text-[10px] text-white/70">efecto mercado vs efecto operado</span>}
                 className="h-full">
            <VariacionTabla varResp={varDelMes} />
          </Panel>
        </div>
      )}
