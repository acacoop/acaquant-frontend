  ...
  return (
    <div className="rounded-md border border-zinc-800 bg-zinc-950">
      <div className="flex items-center justify-between border-b border-zinc-800 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
        <span>Order Book L2 · depth 5</span>
        <span className="font-mono text-[10px] text-amber-400">
          MID {mid != null ? fmt(mid, 4) : "—"}
          {spread != null && (
            <span className="ml-2 text-zinc-500">spread {fmt(spread, 3)}</span>
          )}
        </span>
      </div>
      <table className="w-full text-[12px] tabular-nums">
        <thead>
          <tr className="border-b border-zinc-800 text-[10px] uppercase text-zinc-500">
            <th className="px-3 py-1 text-left font-normal">Bid Size</th>
            <th className="px-3 py-1 text-right font-normal">Bid</th>
            <th className="w-1 px-1 py-1" />
            <th className="px-3 py-1 text-left font-normal">Ask</th>
            <th className="px-3 py-1 text-right font-normal">Ask Size</th>
          </tr>
        </thead>
        <tbody>
          {Array.from({ length: DEPTH }).map((_, i) => {
            ...
            return (
              <tr key={i} className="border-b border-zinc-900">
                <td className="relative px-3 py-1 text-left text-zinc-200">
                  <div
                ...
                <td className="px-3 py-1 text-right font-medium text-emerald-400">
                  {b ? fmt(b.price, 3) : "—"}
                </td>
                <td className="border-x border-amber-500/20 bg-amber-500/5 px-1" />
                <td className="px-3 py-1 text-left font-medium text-rose-400">
                  {o ? fmt(o.price, 3) : "—"}
                </td>
                <td className="relative px-3 py-1 text-right text-zinc-200">
                  <div
