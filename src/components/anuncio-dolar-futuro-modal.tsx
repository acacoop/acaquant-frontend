...
// Anuncio de la NUEVA vista OPERACIONES → DÓLAR FUTURO. Aparece SOLO a los roles
// de la mesa (admin / trader / sales / asistente_comercial) — NUNCA al invitado
// (rol `invitado`). Se muestra una vez por carga dentro de la ventana de
// lanzamiento, salvo que el usuario toque "No volver a mostrar" (localStorage →
// no vuelve nunca). Montado en layout.tsx (toda la app). Contenido estático.
//
// El botón "Ir a Dólar Futuro" deja la sub-pestaña abierta: escribe la clave de
// sessionStorage que usa OperacionesView (usePersistedState "operaciones.tab") y
// navega a /operaciones → la vista rehidrata directo en esa tab.
...
const BLOQUES: { icon: string; titulo: string; desc: string }[] = [
  { icon: "🟢", titulo: "Por tipo de operación", desc: "Compra vs Venta con nocional (US$), arancel (ARS), boletos y % del total." },
  { icon: "👤", titulo: "Por cuenta", desc: "Ranking de comitentes por nocional operado de dólar futuro." },
  { icon: "📅", titulo: "Por instrumento", desc: "Desglose por vencimiento (DLR de cada mes)." },
  { icon: "📊", titulo: "Volumen en el tiempo", desc: "Gráfico de barras del nocional por período, separando compras de ventas." },
];

export function AnuncioDolarFuturoModal() {
  ...
  // Deja la sub-pestaña DÓLAR FUTURO abierta y navega a /operaciones.
  const irADolarFuturo = () => {
    try { sessionStorage.setItem("operaciones.tab", JSON.stringify("dolarfuturo")); } catch { /* noop */ }
    try { localStorage.setItem(KEY, "1"); } catch { /* noop */ }
    setOpen(false);
  };
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="px-6 pt-5 pb-3 border-b border-[var(--t-border)]">
          ...
          <h2 className="text-[22px] font-bold text-[var(--t-text)] leading-tight">
            Nueva vista <span className="text-[var(--t-accent)]">DÓLAR FUTURO</span>
          </h2>
          <p className="text-[12px] text-[var(--t-text-muted)] leading-snug mt-2">
            Dentro de <span className="text-[var(--t-text)] font-semibold">OPERACIONES</span> ya podés chequear
            todo el <span className="text-[var(--t-text)] font-semibold">dólar futuro</span> (futuros DLR de A3):
            el <span className="text-[var(--t-text)] font-semibold">nocional en dólares</span> operado
            (1 contrato = US$ 1.000), el arancel y los boletos, cruzable por tipo, cuenta y vencimiento.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 px-6 py-3 border-t border-[var(--t-border)] bg-[var(--t-surface)]/30">
          ...
          <a href="/operaciones" onClick={irADolarFuturo}
