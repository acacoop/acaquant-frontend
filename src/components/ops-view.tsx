...
// Categorías = `operacion` (derivado del tipo_operacion en CashFlow.Operaciones).
const NEGOCIO_CATS = [
  "compra",
  "venta",
  "caucion_tomadora",
...
const CAT_COLOR: Record<NegocioCat, string> = {
  compra:             "#3fbf6f",
  venta:              "#ff5d6c",
  caucion_tomadora:   "#d09060",
  caucion_colocadora: "#5fd0d0",
  suscripcion:        "#94e7b3",
  rescate:            "#f0a020",
  licitacion:         "#c0a0e0",
  emision:            "#60c0d0",
...
const CAT_LABEL: Record<NegocioCat, string> = {
  compra:             "Compras",
  venta:              "Ventas",
  caucion_tomadora:   "Cauc. Tomadoras",
  caucion_colocadora: "Cauc. Colocadoras",
...
function aggregateSerie(serie: SeriePoint[], agg: AggKey): SeriePoint[] {
  ...
  for (const p of serie) {
    ...
    if (!cur) {
      cur = { fecha: k };
      for (const c of NEGOCIO_CATS) cur[c] = 0;
      m.set(k, cur);
    }
    for (const c of NEGOCIO_CATS) cur[c] = Number(cur[c]) + Number(p[c] ?? 0);
  }
...
export function OpsView() {
  ...
  useEffect(() => {
    const loadFechas = async () => {
      try {
        const res = await fetch("/api/operaciones/ops/fechas", { cache: "no-store" });
  ...
  const fetchData = async (f: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/operaciones/ops/meta?fecha=${f}`, { cache: "no-store" });
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        let url = `/api/operaciones/ops/serie?moneda=${moneda}&cuenta_filter=${filtroCta}`;
        if (mercado) url += `&mercado=${encodeURIComponent(mercado)}`;
  ...
  }, [moneda, mercado, filtroCta, cuentaExacta]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/operaciones/ops/cuentas-list", {
          cache: "no-store",
        });
        if (!res.ok) return;
  ...
  // ── Fetch: lista de mercados (para el dropdown) — una vez ─────────────
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch("/api/operaciones/ops/mercados", { cache: "no-store" });
        if (!res.ok) return;
        const j: { mercados: string[] } = await res.json();
        if (!cancelled) setMercados(Array.isArray(j.mercados) ? j.mercados : []);
      } catch { /* silencioso */ }
    })();
    return () => { cancelled = true; };
  }, []);
  ...
  const totalPeriodo = useMemo(() => {
    let s = 0;
    for (const p of serieRango) for (const c of NEGOCIO_CATS) s += Number(p[c] ?? 0);
    return s;
  }, [serieRango]);

  // Totales para POR CATEGORÍA — bifurcación según vistaMode.
  const totalesActuales = useMemo<Record<string, number>>(() => {
    const acc: Record<string, number> = {};
    for (const c of NEGOCIO_CATS) acc[c] = 0;
    const filas =
      vistaMode === "DIA"
        ? serie.filter((s) => s.fecha === fecha)
        : serieRango;
    for (const p of filas) for (const c of NEGOCIO_CATS) acc[c] += Number(p[c] ?? 0);
    return acc;
  ...
  // DETALLE single-categoría: se deriva del matrix (que ya trae per-cuenta
  // per-categoría) — no hace falta un endpoint aparte.
  const cuentasDetalle = useMemo(
    () =>
      !catSel
        ? []
        : matrix
            .map((r) => ({ cuenta: r.cuenta, importe_abs: Number(r[catSel] ?? 0), n: r.n }))
            .filter((r) => r.importe_abs !== 0)
            .sort((a, b) => b.importe_abs - a.importe_abs),
    [catSel, matrix],
  );
  const loadingCuentas = loadingMatrix;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        let url =
          `/api/operaciones/ops/cuentas-matrix?moneda=${moneda}` +
    ...
    return () => { cancelled = true; };
  }, [moneda, mercado, filtroCta, detalleDesde, detalleHasta, cuentaExacta]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        let url =
          `/api/operaciones/ops/boletos?fecha=${fecha}` +
        ...
        if (cancelled) return;
        const lista = Array.isArray(j.boletos) ? j.boletos : [];
        const filtrados = catSel ? lista.filter((b) => b.operacion === catSel) : lista;
        setBoletosCache((prev) => ({ ...prev, [key]: filtrados }));
      } catch (e) {
        if (!cancelled) {
          setError(e instanceof Error ? e.message : String(e));
        }
    ...
    return () => { cancelled = true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [expandedCuenta, vistaMode, fecha, moneda, mercado, catSel]);
    ...
    <div className="h-full overflow-hidden bg-[var(--t-panel)] text-[var(--t-text)] flex flex-col">
      ...
      <div className="px-4 py-3 border-b border-[var(--t-border)] flex flex-wrap items-center gap-3 shrink-0">
        ...
        <select
          ...
          className="bg-[var(--t-panel)] border border-[var(--t-border-2)] px-2 py-1 text-[11px] text-[var(--t-text)] outline-none [color-scheme:dark]"
      ...
      {fechasLoaded && fechasDisp.length === 0 && !loading && (
        <div className="m-4 border border-[var(--t-border)] p-8 text-center text-[12px] text-[var(--t-text-muted)]">
          Aún no hay datos en CashFlow.Operaciones. Corré el backfill + enrich.
        </div>
      )}
                                ...
                                {!boletosForRow ? (
                                  ...
                                  <table className="w-full text-[10px] font-mono tabular-nums">
                                    <thead className="text-[8px] uppercase tracking-widest text-[var(--t-text-muted)]">
                                      <tr>
                                        <th className="px-3 py-1 text-left">Boleto</th>
                                        <th className="px-2 py-1 text-left">Operación</th>
                                        <th className="px-2 py-1 text-left">Mercado</th>
                                        <th className="px-2 py-1 text-left">Instrumento</th>
                                        <th className="px-2 py-1 text-right">Cantidad</th>
                                        <th className="px-2 py-1 text-right">Bruto</th>
                                        <th className="px-2 py-1 text-left">Condiciones</th>
                                      </tr>
                                    </thead>
                                    <tbody>
                                      {boletosForRow.map((b) => {
                                        const catColor = CAT_COLOR[b.operacion as NegocioCat] ?? "#666";
                                        return (
                                          <tr
                                            key={b.boleto}
                                            className="border-t border-[var(--t-border)]"
                                          >
                                            <td className="px-3 py-0.5 text-[var(--t-text-dim)]">{b.boleto}</td>
                                            <td className="px-2 py-0.5">
                                              <span className="inline-flex items-center gap-1">
                                                <span className="w-1.5 h-1.5 inline-block" style={{ background: catColor }} />
                                                <span className="text-[var(--t-text-dim)]">{b.operacion ?? "—"}</span>
                                              </span>
                                            </td>
                                            <td className="px-2 py-0.5 text-[var(--t-text)]">{b.mercado || "—"}</td>
                                            <td className="px-2 py-0.5 text-[var(--t-accent)] truncate max-w-[200px]" title={b.instrumento ?? ""}>{b.instrumento ?? "—"}</td>
                                            ...
                                            <td className="px-2 py-0.5 text-right text-[var(--t-text)]">
                                              {b.bruto != null
                                                ? b.bruto.toLocaleString("es-AR", { maximumFractionDigits: 2 })
                                                : "—"}
                                            </td>
                                            <td className="px-2 py-0.5 text-[var(--t-text-dim)]">{b.condiciones ?? "—"}</td>
                                          </tr>
                                        );
                                      })}
                                    </tbody>
