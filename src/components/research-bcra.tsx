...
// Tab BCRA de la vista RESEARCH — doc vivo: TRD-FX docs/RESEARCH_BCRA.md.
// SUB-TABS por BLOQUE (cada una con su propia lógica/series/unidad): Reservas ·
// Tipo de cambio · Tasas · Dinero · Inflación · CER & UVA · Depósitos.
// Diseño = el header único de los otros charts de Research. Eficiencia: el
// catálogo de bloques se pide UNA vez; cada bloque fetchea sus series en UNA
// request batch, lazy al abrirlo (keep-alive: no re-fetchea al volver).
...
interface BcraSerieMeta { id: number; etiqueta: string; unidad: string | null; desde: string | null; hasta: string | null }
interface BcraBloques { bloques: { bloque: string; series: BcraSerieMeta[] }[] }
...
const BLOQUE_ORDEN = ["reservas", "tipo_cambio", "tasas", "dinero", "inflacion", "indexacion", "depositos"];
...
function fmtValor(v: number, unidad: string | null): string {
  ...
  if (unidad === "M USD" || unidad === "M ARS") {
    // vienen en millones → mostrar compacto (48.617 M / 33,2 B)
...
function BcraBloque({ bloque, series }: { bloque: string; series: BcraSerieMeta[] }) {
  ...
  const cargar = useCallback(async () => {
    ...
    setErr(null);
    try {
      const qs = idsActivas.map((i) => `ids=${i}`).join("&");
      const r = await fetch(`/api/research-bcra/series?${qs}&desde=${desdeISO(dias)}`, { cache: "no-store" });
  ...
  return (
    <section className="h-full min-h-0 flex flex-col">
      {/* header único — mismo diseño que los charts de Argentina */}
        ...
        {series.map((s, i) => {
          ...
          return (
            <button key={s.id} type="button" onClick={() => toggle(s.id)}
              title={s.desde ? `historia desde ${s.desde}` : "sin datos sincronizados aún"}
...
// ── La tab BCRA: sub-tabs por bloque (keep-alive) ────────────────────────────
export function ResearchBcra() {
  const [data, setData] = useState<BcraBloques | null>(null);
  const [err, setErr] = useState<string | null>(null);
  const [sub, setSub] = useState<string>("reservas");
  const [visitados, setVisitados] = useState<Set<string>>(() => new Set(["reservas"]));
  if (!visitados.has(sub)) setVisitados(new Set(visitados).add(sub));

  useEffect(() => {
    let vivo = true;
    fetch("/api/research-bcra/bloques")
      .then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
      .then((d: BcraBloques) => { if (vivo) setData(d); })
      .catch((e) => { if (vivo) setErr(`No pude cargar los bloques del BCRA (${e.message}).`); });
    return () => { vivo = false; };
  }, []);

  const bloques = useMemo(() => {
    const map = new Map((data?.bloques || []).map((b) => [b.bloque, b.series]));
    return BLOQUE_ORDEN.filter((b) => map.has(b)).map((b) => ({ bloque: b, series: map.get(b)! }));
  }, [data]);
  ...
  if (err || (data && bloques.length === 0)) {
    return (
      <div className="h-full flex items-center justify-center text-[11px] text-center px-6"
           style={{ color: err ? "var(--t-neg)" : "var(--t-text-dim)" }}>
        {err ?? "Sin series del BCRA sincronizadas todavía — corré `python -m jobs.bcra_research --backfill` en el Droplet."}
      </div>
    );
  }
  if (!data) {
        ...
        {bloques.map(({ bloque, series }) =>
          visitados.has(bloque) ? (
            <div key={bloque} className={sub === bloque ? "h-full" : "hidden"}>
              <BcraBloque bloque={bloque} series={series} />
            </div>
          ) : null,
        )}
      </div>
