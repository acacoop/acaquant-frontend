...
/**
 * COBERTURAS — análisis de un trade puntual + buscador de coberturas.
 *
...
export function CoberturasView() {
  ...
  const [ticker, setTicker] = usePersistedState<string | null>("estrategia.cob.ticker", null);
  const [monto, setMonto] = usePersistedState<number>("estrategia.cob.monto", 1_000_000);
  const [direccion, setDireccion] = usePersistedState<Direccion>("estrategia.cob.dir", "long");
    ...
    <div className="h-full min-h-0 flex flex-col p-3 gap-2 overflow-y-auto">
      ...
      <div className="flex items-center gap-2 flex-wrap shrink-0">
        ...
        <span className="flex items-center gap-1">
          <span className="text-[9px] text-[var(--t-text-muted)] tracking-widest">PLATA EN JUEGO (USD)</span>
          <input
      ...
      {!ticker && (
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center">
          ...
          <p className="text-[var(--t-text-dim)] text-xs max-w-[420px]">
            Elegí un papel, poné cuánta plata pondrías, y te muestra qué riesgo
            tiene ese trade y con qué otro papel podés cubrirte si se da vuelta.
          </p>
                  ...
                  <table className="w-full text-[11px]">
                    <thead className="sticky top-0 bg-[var(--t-panel)] border-b border-[var(--t-border-2)]">
                      <tr className="text-[var(--t-text-muted)] text-[9px] tracking-wider">
                        ...
                        <th className="!px-2 !py-1 text-right">CUÁNTO</th>
                        <th className="!px-2 !py-1 text-center">ACCIÓN</th>
                        <th className="!px-2 !py-1 text-right">RIESGO QUE SACA</th>
                      </tr>
                    </thead>
                    <tbody>
                      {candidatos.map((h, i) => (
